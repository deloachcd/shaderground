// Original: https://www.shadertoy.com/view/MslGRn
...
void main() {
    ...
    vec3 oo = vec3(0, 0, 1.0-u_mouse.y/u_resolution.y);
    vec3 od = normalize(vec3(p.x, p.y, -2.0));
    vec3 o,d;   
    o.xz = rt(oo.xz, 6.3*u_mouse.x/u_resolution.x);
    o.y = oo.y;
    d.xz = rt(od.xz, 6.3*u_mouse.x/u_resolution.x);
    d.y = od.y;
    ...
    {   
        ...
        for (float i = 0.0; i < SLICES; i+=1.0)
        {
            ...
            for (int j = 0; j < 3; j++)                                     
            {
                d += 0.5/abs((fbm(5.0*curpos*b+ANIMATION_SPEED*u_time/b)*2.0-1.0)/a);
                b *= 2.0;
                a /= 2.0;
            }
